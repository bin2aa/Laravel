        ...
        <form action="{{ route('users') }}" method="GET" class="mb-4">
            <div class="input-group">
                <select name="search_type" class="form-select">
                    <option value="nameSearch" {{ request('search_type') == 'nameSearch' ? 'selected' : '' }}>Tìm kiếm theo
                        tên</option>
                    <option value="emailSearch" {{ request('search_type') == 'emailSearch' ? 'selected' : '' }}>Tìm kiếm theo
                        email</option>
                </select>
